...
def interpretar_resultados(label, best_fitness):
    ...
    resposta = summarizer(prompt, max_length=150, min_length=30, do_sample=False)[0]["generated_text"]
...
def save_results(label, config, best_params, best_fitness, fitness_history):
    ...
    plt.plot(range(1, len(fitness_history) + 1), fitness_history, marker="o", color="red")
...
if __name__ == "__main__":
    print("Limpando resultados anteriores de Câncer de Mama...")
    purge_results("Câncer_de_Mama")

    configs = [
        {"population_size": 6, "generations": 5, "mutation_rate": 0.1},
        {"population_size": 10, "generations": 8, "mutation_rate": 0.2},
        {"population_size": 12, "generations": 10, "mutation_rate": 0.3},
    ]
    for i, config in enumerate(configs, 1):
        run_experiment(config, label=f"Câncer de Mama - Config {i}")
